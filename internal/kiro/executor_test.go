...
func TestNewExecutor(t *testing.T) {
	executor := NewExecutor("/custom/path/kiro", 5*time.Minute, nil)
...
func TestExecute_InvalidBinary(t *testing.T) {
	executor := NewExecutor("nonexistent-binary-12345", 1*time.Minute, nil)
...
func TestExecute_WithSessionID(t *testing.T) {
	...
	executor := NewExecutor("echo", 1*time.Minute, nil)
